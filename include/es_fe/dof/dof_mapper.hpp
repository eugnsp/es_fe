...
#include <es/fe/util/algorithm.hpp>
...
private:
	...
	{
 		static_assert(Var_t<var>::Element::has_vertex_dofs,
	...
	{
		static_assert(Var_t<var>::Element::has_dof(Edge_tag{}),
			"Variable has no DoF of this type");
	...
	void compute_n_dofs(const System& system)
	{
		...
		for_each_var_element<Var_list>(
			[this, &system](auto var, auto element_tag)
			{
 				n_dofs_ += system.variable(var).n_dofs(element_tag) *
					*n_elements(system.mesh(), element_tag);
 			});
	}
	...
	void assign_indices(const System& system)
	{
		...
		for_each_var_element<Var_list>(
			[this, &system, &free_index, &const_index](auto var, auto element_tag)
			{
				...
				for (Element_index element{}; element < n_elements(system.mesh(), element_tag); ++element)
				{
		...
		Face_index cell, Dofs& dofs) const
	{
		...
		if constexpr (Var::Element::has_edge_dofs)
			for (const auto& halfedge : halfedges)
				var_dofs_impl2<var>(edge(halfedge), first_degree_dofs, i, is_first_halfedge(halfedge));
	...
	void var_dofs_impl2(Element_index element, Dofs& dofs,
		std::size_t& i, bool reversed = false) const
	{
		...
		for (Local_index k = 0; k < n; ++k)
		{
			...
			dofs[i++] = first_dof + static_cast<Local_index>(j) * Var::dim(); // HACK for static-dim vars
		}
	}
