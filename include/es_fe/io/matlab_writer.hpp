...
public:
	...
	void write_face_scalar_field(const std::string& var_name, const Field& field)
	{
		...
		la::Vector_xd vertex_field(*mesh_.n_vertices(), 0), norm(*mesh_.n_vertices(), 0);
...
private:
	...
	{
		// Vertices
		la::Matrix_xd vertices(2, *mesh_.n_vertices());
		...
		// Faces
		la::Matrix_x<Index> tri_faces(3, *mesh_.n_faces());
