...
bool contains(const Triangle& triangle, const Point& point, Triangle_tag, Point_tag)
{
...
bool contains(const Rect& rect, const Point& point, Rect_tag, Point_tag)
{
...
bool contains(const Linestring& linestring, const Point& point, Linestring_tag, Point_tag)
{
	return es_util::any_of_pairs(
		linestring.begin_vertex(), linestring.end_vertex(),
		[&point](auto& v1, auto& v2) { return is_point_on_line_segment(point, v1, v2); });
}

template<class Polygon, class Point>
bool contains(const Polygon& poly, const Point& point, Polygon_tag, Point_tag)
{
	bool inside = false;
