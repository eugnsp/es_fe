...
// 	void vertex_indices(std::array<Index, 2>&) const;
	...
	Vertex_index vertex_from_index() const{throw;}			// = vertex_index(0)
 	Vertex_index vertex_to_index() const{throw;}				// = vertex_index(1)

 	const geom::Point& vertex(Local_index) const{throw;}
	...
	//Vertex_view vertex_view(Local_index) const;
 	Vertex_view vertex_from_view() const{throw;}		// = vertex_view(0)
 	Vertex_view vertex_to_view() const{throw;}			// = vertex_view(1)
