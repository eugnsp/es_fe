...
#include <es/fe/math/dunavant/data.hpp>
...
private:
	// Returns the index of the first point in the group with the index (group_index)
...
using Dunavant_quadr_t = Dunavant_quadr<order, Make_local_index_sequence<
	std::tuple_size_v<decltype(Dunavant_data<order>::groups)>>>;
}
